...
│   │   ├── handlers/         # 9 个方法的处理函数
│   │   └── path_utils.py     # 用户输入路径归一化（file:// URI → Path）
...
| `probe/path_utils.py` | 路径归一化工具：`path_from_user_input()` 把用户输入的本地路径或 `file://` URI 统一转成 `Path`，被 import / scan / reader 复用（替代裸 `Path(x).expanduser()`） |
